...
export default function ShipmentExecutionSection({
  shipments = [],
  loading,
  onAddShipment,
  onRemoveShipment,
  ...
  if (loading) {
    return (
      <div className="bg-white border border-gray-200 rounded-2xl p-8 text-center animate-pulse">
        ...
        <span className="text-xs font-semibold text-gray-500">Loading shipment matrix...</span>
      </div>
    );
  }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-100 pb-3">
        <div>
          <h3 className="text-xs font-bold text-gray-900 uppercase tracking-wider flex items-center gap-2">
            ...
            Shipment Execution Matrix ({shipments.length} Active Shipments)
          </h3>
          <p className="text-[11px] text-gray-500">
            Live joined operational metrics from Sales Contract Shipment records. Click any row to view complete dispatch details.
          </p>
        </div>
        ...
        {unlinkedShipments.length > 0 && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-purple-700 bg-purple-50 hover:bg-purple-100 border border-purple-200 rounded-xl transition-colors self-start sm:self-auto"
          >
