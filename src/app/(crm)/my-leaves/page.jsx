...
function MyLeavesContent() {
  ...
  const submitApplyLeave = async (e) => {
    ...
    try {
      ...
      showToast(err || "Failed to apply leave", "error");
    } finally {
  ...
  const handleCancelLeave = async () => {
    ...
    try {
      ...
      showToast(err || "Failed to cancel leave", "error");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1200px] mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {balances.map(bal => (
          <div key={bal.id} className="bg-white rounded-2xl border border-gray-100 p-4 flex flex-col justify-between shadow-xs">
            <span className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">{bal.leaveType?.name}</span>
            <div className="mt-2 flex items-baseline gap-2">
              <span className="text-2xl font-black text-gray-800">{bal.balance}</span>
              <span className="text-xs font-medium text-gray-400">days left</span>
            </div>
            <div className="mt-1 text-[10px] text-gray-400">
              Total {bal.allottedDays} • Used {bal.usedDays} • Pending {bal.pendingDays}
            </div>
          </div>
        ))}
        {balances.length === 0 && (
