            ...
            return partners.length > 0 ? (
              partners.map((item) => {
                const dueTodayCount = item.followUps?.filter(f => {
                  ...
                  return localNextDate <= today && !completedStatuses.includes(f.status);
                }).length || 0;
                ...
                return (
                  ...
                  >
                    <td className="px-6 py-4">
                      ...
                      <div className="text-[11px] text-[#007aff] font-medium mt-0.5 flex items-center gap-2">
                        <span>{item.partnerRole?.name || "-"}</span>
                        {item.yearOfEstablishment && (
                          <span className="text-[10px] text-gray-500 font-semibold bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200">
                            Est. {item.yearOfEstablishment}
                          </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right space-x-2" onClick={(e) => e.stopPropagation()}>
                      ...
                      <HasPermission permission="follow_up:create">
                        <button
                          ...
                          className="p-1 rounded-lg text-gray-500 hover:text-purple-600 hover:bg-purple-50 transition-colors cursor-pointer"
                          title="Follow Up"
                        >
