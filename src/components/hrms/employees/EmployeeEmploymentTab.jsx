  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-2xs overflow-hidden">
      ...
      <div className="p-4 space-y-3.5 text-xs">
        ...
        <div className="flex justify-between"><span className="text-gray-400 font-medium">Type</span><span className="font-bold text-gray-800">{empDetails.employmentType?.replace('_', ' ')?.toLowerCase()?.replace(/\b\w/g, c => c.toUpperCase()) || "-"}</span></div>
      </div>
    </div>
  );
}
